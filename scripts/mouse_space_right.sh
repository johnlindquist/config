...
DEBUG=false
LOG_FILE="$HOME/.config/logs/mouse_space_right.log"
...
if [[ "$DEBUG" == "true" ]]; then
  ...
  exec 2>>"$LOG_FILE"
  log "--- Script Start: mouse_space_right.sh ---"
fi
...
log "Calculating next space index relative to $current_visible_index..."
...
if [[ -z "$next_space_index" ]]; then
  log "Could not determine the next space index. This might mean there is only one space on display $target_display_id."
  ...
  if [[ "$space_count" -le 1 ]]; then
      ...
      log "Error: Failed to determine next space index, but multiple spaces ($space_count) exist. Logic error?"
...
if [[ "$next_space_index" == "$current_visible_index" ]]; then
     log "Next space index ($next_space_index) is the same as current ($current_visible_index). No action needed."
     exit 0
fi

log "Determined next space index on display $target_display_id: $next_space_index"

# 5. Focus the Next Space
log "Focusing space index $next_space_index..."
if "$YABAI_PATH" -m space --focus "$next_space_index"; then
  log "Focus command executed successfully."
else
  log "Error: yabai focus command failed for index $next_space_index."
  # Attempt to query the space to see if it exists
  space_info=$("$YABAI_PATH" -m query --spaces --space "$next_space_index")
  log "Queried space info for index $next_space_index: $space_info"
  echo "Error: Failed to focus space $next_space_index." >&2
  exit 1
fi

log "--- Script End: mouse_space_right.sh ---"
exit 0 