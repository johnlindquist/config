...
# --- Script Setup ---
SCRIPT_NAME="move-window-east"
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
LOGGER_SCRIPT_PATH="$SCRIPT_DIR/log_helper.sh"

# Log the start of the script
"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "START" "Script execution started - Move window to next column (east)"
...
"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "ACTION" "Attempting to focus window to the east"
$y -m window --focus east

# Check if focus changed
"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "INFO" "Checking if focus changed"
NEW_FOCUSED_WINDOW_ID=$($y -m query --windows --window | $jq -r '.id')
"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "INFO" "Window ID after attempting focus east: $NEW_FOCUSED_WINDOW_ID"

if [ "$FOCUSED_WINDOW_ID" == "$NEW_FOCUSED_WINDOW_ID" ]; then
  "$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "INFO" "Focus did not change. Window might be the rightmost or focus failed"
  log_state "AFTER"
...
if [ -z "$NEW_FOCUSED_WINDOW_ID" ] || [ "$NEW_FOCUSED_WINDOW_ID" == "null" ]; then
  "$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "ERROR" "Failed to get new focused window ID after focus east"
...
"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "ACTION" "Swapping original window $FOCUSED_WINDOW_ID with eastern window $NEW_FOCUSED_WINDOW_ID"
