...
# ---------- focus_previous_window_same_app.sh ----------
# This script focuses on the previous window of the currently focused app
# Cycles backwards through windows of the same app across all spaces

# Add logging helper path
LOGGER_SCRIPT_PATH="$(dirname "$0")/log_helper.sh"
SCRIPT_NAME="focus_previous_window_same_app.sh"
...
PREVIOUS_INDEX=$(( (CURRENT_INDEX - 1 + WINDOW_COUNT) % WINDOW_COUNT ))

# Get previous window ID
PREVIOUS_WINDOW_ID=$(echo "$ALL_APP_WINDOWS" | $JQ -r ".[$PREVIOUS_INDEX].id")
PREVIOUS_WINDOW_SPACE=$(echo "$ALL_APP_WINDOWS" | $JQ -r ".[$PREVIOUS_INDEX].space")

"$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "ACTION" "Switching to previous window: ID=$PREVIOUS_WINDOW_ID, Space=$PREVIOUS_WINDOW_SPACE"
...
if [[ $focus_exit -eq 0 ]]; then
    "$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "INFO" "Successfully focused window $PREVIOUS_WINDOW_ID"
else
    "$LOGGER_SCRIPT_PATH" "$SCRIPT_NAME" "ERROR" "Failed to focus window $PREVIOUS_WINDOW_ID (exit code: $focus_exit)"
fi
