...
DEBUG=false
LOG_FILE="$HOME/.config/logs/mouse_space_left.log"
...
if [[ "$DEBUG" == "true" ]]; then
  # Redirect stderr to the log file if debugging is enabled
  exec 2>>"$LOG_FILE"
  log "--- Script Start: mouse_space_left.sh ---"
fi

# Check for required commands
if ! command -v "$YABAI_PATH" &> /dev/null; then
  echo "Error: yabai not found at $YABAI_PATH" >&2
  log "Error: yabai not found at $YABAI_PATH"
  exit 1
fi
...
if ! command -v "$CLICLICK_PATH" &> /dev/null; then
  ...
  log "Error: cliclick not found at $CLICLICK_PATH"
  exit 1
fi
...
log "Getting mouse coordinates..."
coords=$("$CLICLICK_PATH" q:.) # Query format: x,y
if [[ ! "$coords" =~ ^[0-9]+\.?[0-9]*,[0-9]+\.?[0-9]*$ ]]; then
    log "Error: Failed to get valid mouse coordinates from cliclick. Output: $coords"
...
log "Mouse coordinates: X=$mouse_x, Y=$mouse_y"

# 2. Get Display Info and Find Target Display
log "Querying display information..."
displays_json=$("$YABAI_PATH" -m query --displays)
if [[ -z "$displays_json" ]]; then
    log "Error: Failed to query displays using yabai."
    echo "Error: Could not query displays." >&2
    exit 1
fi
log "Raw display JSON: $displays_json"
...
if [[ -z "$target_display_id" ]]; then
  log "Error: Could not determine display for mouse coordinates ($mouse_x, $mouse_y)."
  log "Available displays: $displays_json"
  echo "Error: Could not determine which display the mouse is on." >&2
  exit 1
fi
log "Mouse is on display ID: $target_display_id"

# 3. Find Current Visible Space and All Spaces on Target Display
log "Querying spaces for display $target_display_id..."
spaces_on_display_json=$("$YABAI_PATH" -m query --spaces --display "$target_display_id")
if [[ -z "$spaces_on_display_json" ]]; then
    log "Error: Failed to query spaces for display $target_display_id."
    echo "Error: Could not query spaces for the target display." >&2
    exit 1
fi
log "Spaces on target display JSON: $spaces_on_display_json"
...
if [[ -z "$current_visible_index" ]]; then
    log "Error: Could not find a visible space on display $target_display_id. Query result: $spaces_on_display_json"
    ...
    if [[ -z "$current_visible_index" ]]; then
        log "Error: Fallback failed. No spaces found on display $target_display_id."
        echo "Error: No spaces found on the target display." >&2
        exit 1
    fi
    log "Fallback: Using first space index $current_visible_index as current."
fi
log "Current visible space index on display $target_display_id: $current_visible_index"

# 4. Determine the Previous Space Index
log "Calculating previous space index relative to $current_visible_index..."
...
if [[ -z "$previous_space_index" ]]; then
  log "Could not determine the previous space index. This might mean there is only one space on display $target_display_id."
  # Check if there really is only one space
  space_count=$("$JQ_PATH" 'length' <<< "$spaces_on_display_json")
  if [[ "$space_count" -le 1 ]]; then
      log "Confirmed: Only one space ($space_count) exists on display $target_display_id. No action needed."
      echo "Only one space on this display." # User-friendly message
      exit 0
  else
      log "Error: Failed to determine previous space index, but multiple spaces ($space_count) exist. Logic error?"
...
if [[ "$previous_space_index" == "$current_visible_index" ]]; then
     log "Previous space index ($previous_space_index) is the same as current ($current_visible_index). No action needed."
     exit 0
fi

log "Determined previous space index on display $target_display_id: $previous_space_index"

# 5. Focus the Previous Space
log "Focusing space index $previous_space_index..."
if "$YABAI_PATH" -m space --focus "$previous_space_index"; then
  log "Focus command executed successfully."
else
  log "Error: yabai focus command failed for index $previous_space_index."
  # Attempt to query the space to see if it exists
  space_info=$("$YABAI_PATH" -m query --spaces --space "$previous_space_index")
  log "Queried space info for index $previous_space_index: $space_info"
  echo "Error: Failed to focus space $previous_space_index." >&2
  exit 1
fi

log "--- Script End: mouse_space_left.sh ---"
exit 0 